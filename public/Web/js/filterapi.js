$(document).ready(function () {
    ...
    var path = h.pathname;
    var url = "";
    if (path === '/home' || path === '/home/' || path === '' || path === '/') {
        url = "/api/v1/product-filter?filter=(;limit=8;currentPage=1;)";
    } else if (path.startsWith('/home/') && path.length > 6) {
        var match = path.match(/^\/home\/(.+)/);
        if (match) {
            var extractedString = match[1];
            url = "/api/v1/" + extractedString + "/product-filter?filter=(;limit=8;currentPage=1;)";
        }
    }
...
function updateUrl(isChangeSelectTotal = false) {
    ...
    var url = "";
    if (path === '/home' || path === '/home/' || path === '' || path === '/') {
        url = "/api/v1/product-filter?filter=(;";
    } else if (path.startsWith('/home/') && path.length > 6) {
        var match = path.match(/^\/home\/(.+)/);
        if (match) {
            var extractedString = match[1];
            url = "/api/v1/" + extractedString + "/product-filter?filter=(;";
        }
    }
                        ...
                        <div class="block2">
                            <div style="border: 1px solid #ccc" class="block2-pic hov-img0">
                                ...
                                <a style="border: 1px solid #ccc; font-size: 13px;"
                                    href="${'/' + product.id + '/productDetail'}" class="block2-btn flex-c-m stext-103 cl2 size-102 bg0 bor2 hov-btn1 p-lr-15 trans-04 
                                    ">
                                    Chi tiết sản phẩm
                                </a>
                            </div>
